...
praesidium_vectors:
    ...
    // Lower EL, aarch64 — userspace (EL0) exceptions.
    .balign 0x80
    b el0_sync_stub          // 0x400 synchronous (SVC syscall / EL0 fault)
    .balign 0x80
    b {unexpected}           // 0x480 IRQ (EL0 masks IRQ in P7a; preemptible userspace is a follow-on)
    .balign 0x80
    b {unexpected}           // 0x500 FIQ
    .balign 0x80
    b {unexpected}           // 0x580 SError
    // Lower EL, aarch32.
    .balign 0x80
    b {unexpected}
    .balign 0x80
...
// EL0 synchronous trap (SVC syscall / userspace fault). Same full-frame save as the EL1 stub —
// SP switched to SP_EL1 (the process's kernel stack) on entry; SP_EL0 (the user stack) is banked
// and preserved. The handler dispatches the syscall (result written into the saved x0) or kills
// the process (never returning); on a syscall it returns and the shared epilogue erets to EL0.
el0_sync_stub:
    sub sp, sp, #272
    stp x0, x1, [sp, #16*0]
    stp x2, x3, [sp, #16*1]
...
"#,
    ...
    el0_sync_handler = sym super::user::el0_sync_handler,
);
